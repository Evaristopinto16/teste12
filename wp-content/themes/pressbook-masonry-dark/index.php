...
?>
	...
	<div class="pb-content-sidebar u-wrapper">
		<main id="primary" class="<?php echo esc_attr( PressBook_Masonry_Dark_Options::blog_site_main_class() ); ?>">
		...
		if ( have_posts() ) {
			if ( is_home() && ! is_front_page() ) {
				...
				?>
				<header class="<?php echo esc_attr( $pressbook_archv_title['header'] ); ?>">
					<h1 class="<?php echo esc_attr( $pressbook_archv_title['class'] ); ?>"><?php /* translators: %s: search query */ printf( esc_html__( 'Search Results for: %s', 'pressbook-masonry-dark' ), '<span>' . esc_html( get_search_query() ) . '</span>' ); ?></h1>
				</header>
				<?php
			}
